...
Ohjelmatyökalu, jota voidaan käyttää kaveripiirin analysointiin
"""
...
def read_friendship_network(filename):
    ...
    network = {}    # Tehdään kaveripiiristä sanakirja, joka sisältää avaimena
                    # henkilön nimen ja arvona henkilön kaverit listana
...
def add_friendship(network, name1, name2):
    ...
    if name1 != name2:
        # Lisätään nimet sanakirjaan tarvittaessa
        ...
        # Lisätään kaverisuhde tarvittaessa
        if name2 in network[name1]:
            ...
            # Järjestetään kaverilistat tässä kohtaa
            network[name1].sort()
            network[name2].sort()
...
def print_command(network, namelist):
    ...
    # Järjestetään network-sanakirja tässä kohtaa
    sorted_dict = {}

    for name in sorted(network):
        sorted_dict[name] = network[name]

    # Tulostetaan aakkosjärjestyksessä oleva sanakirja
    for name, friends in sorted_dict.items():
        print(name)
...
def common_friends_command(network, namelist):
    ...
    commons_list = []   # Lista yhteisistä kavereista, jotta voidaan tarkastaa
                        # yhteisten ystävien olemassaolo


    for name in network[name1]:
        if name in network[name2]:
            commons_list.append(name)

    if len(commons_list) > 0:
        for name in commons_list:
            print(name)
