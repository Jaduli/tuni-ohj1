...
Vaatii tiedoston laivan koordinaateista.
"""
...
def print_board(hit_coords, miss_coords, sunk_coords):
    """
    Tulostaa laivanupotuksen pelilaudan.
    ...
    for sunked in sunk_coords:
        # jos laiva on upotettu, X:n sijaan tulostetaan kohtaan laivan tyypin
        # ensimmäinen kirjain
        column = ord(sunked[0]) - ord("A") + 1
        row = int(sunked[1])

        rows_list[row][column] = sunked[2]
...
def check_input(input):
    ...
    # Oletetaan, että syöte on sopiva. Muutetaan oletusta tarvittaessa
    check = True
